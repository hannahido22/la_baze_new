// Bulles réseaux sociaux — en haut de page, en couleur.
// Positionnées en absolu (elles restent en haut et défilent avec la page,
// elles ne "suivent" pas le scroll). Pour ajouter un réseau (TikTok, YouTube…),
// ajoute une entrée ci-dessous.

type Social = {
  label: string;
  href: string;
  bg: string; // fond coloré de la marque
...
export default function SocialBubbles() {
  return (
    <div className="absolute right-3 sm:right-4 top-20 sm:top-24 z-40 flex flex-col gap-2.5 sm:gap-3">
      {socials.map((s) => (
        <a
          key={s.label}
          href={s.href}
          ...
          className="flex items-center justify-center w-9 h-9 sm:w-10 sm:h-10 rounded-full text-white shadow-md shadow-black/30 ring-1 ring-white/15 transition-transform duration-300 hover:scale-110"
        >
          {s.icon}
        </a>
      ))}
