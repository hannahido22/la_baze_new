// Bulles réseaux sociaux flottantes — fixées en haut à droite, toujours visibles.
// Pour ajouter un réseau (TikTok, YouTube…) : ajoute une entrée ci-dessous.

type Social = {
  label: string;
  href: string;
  color: string; // couleur de la marque (au survol)
...
export default function SocialBubbles() {
  return (
    <div className="fixed right-2.5 sm:right-4 top-24 sm:top-28 z-40 flex flex-col gap-2.5 sm:gap-3">
      {socials.map((s, i) => (
        <div
          key={s.label}
          style={{ animation: 'bubble-float 4s ease-in-out infinite', animationDelay: `${i * 0.4}s` }}
        >
          <a
            href={s.href}
            target="_blank"
            ...
            aria-label={s.label}
            className="group flex items-center justify-center w-9 h-9 sm:w-11 sm:h-11 rounded-full bg-[#140a0f]/80 border border-white/15 text-white/70 shadow-lg shadow-black/30 transition-all duration-300 hover:scale-110 hover:border-white/40"
            style={{ ['--brand' as string]: s.color }}
          >
            <span className="transition-colors duration-300 group-hover:text-[var(--brand)]">
              {s.icon}
            </span>
          </a>
        </div>
